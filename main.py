...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in WAITING_FOR_SS:
        ...
        await context.bot.send_photo(
            chat_id=ADMIN_ID,
            photo=photo_file_id,
            caption=f"🔔 <b>New Payment Verification!</b>\n\n👤 User ID: <code>{user_id}</code>\n📦 Product: <b>{stock_key}</b>\n\nकृपया बैंक में चेक करके Approve/Reject करें।",
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # जब यूजर Done Payment पर क्लिक करता है
        ...
        await query.edit_message_caption(
            caption="📸 <b>कृपया अपने सफल पेमेंट का Screenshot यहाँ भेजें!</b>\n\n(फोटो गैलरी से सेलेक्ट करके सेंड करें)", 
            parse_mode="HTML"
        )
        ...
        if KEYS.get(stock_key) and len(KEYS[stock_key]) > 0:
            ...
            try:
                await context.bot.send_message(
                    ...
                    text=f"🎉 <b>पेमेंट सफल! एडमिन द्वारा अप्रूव्ड।</b>\n\nयहाँ आपकी Key है: <code>{key}</code>\n\n(यह Key आपके 'My Licences' में सेव हो गई है)", 
                    parse_mode="HTML"
                )
            except:
                pass
    ...
    # जब एडमिन 'Reject' बटन दबाए
        ...
        await query.edit_message_caption(caption=f"❌ <b>Rejected!</b>\nUser {target_user_id} को रिजेक्ट मैसेज भेज दिया गया है।", parse_mode="HTML")
